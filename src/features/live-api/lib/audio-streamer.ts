...
export class AudioStreamer {
  ...
  async addWorklet<T extends (event: MessageEvent) => void>(
    workletName: string,
    workletSrc: string,
    handler: T,
  ): Promise<this> {
  ...
  private scheduleNextBuffer() {
    ...
    while (
      ...
      if (worklets) {
        Object.values(worklets).forEach((graph) => {
          ...
          if (node) {
            source.connect(node);
            node.port.onmessage = function onMessage(ev: MessageEvent) {
    ...
    if (this.audioQueue.length === 0) {
      ...
      window.setTimeout(() => this.scheduleNextBuffer(), Math.max(0, nextCheckTime - 50));
    }
  }
